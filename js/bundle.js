...
function initSearch() {
  ...
  $(".search-toggle").click(function (e) {
    ...
    if (searchClosed) {
      $("html, body").animate({
        ...
        if (firstCallback) {
          search.toggleClass("expanded"); // give container max-size based on content
      ...
      // if search is open
      search.toggleClass("expanded"); // set container max-height back to 0
  ...
  $(".search-input").keyup(function () {
    ...
    searchText = $(this).val().toLowerCase(); // remove match/not-match classes

    searchItems.removeAttr("class");

    if (searchText !== "") {
      // find matches
      $('.search-list li[data-search*="' + searchText + '"]').addClass("match"); // find not-matches
